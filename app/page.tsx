...
export default function CharacterBuilder() {
  ...
  const handleExportPDF = () => {
    ...
    // Wait for React to re-render and remove the watermark to avoid CORS crash
    setTimeout(async () => {
      try {
        if (!sheetRef.current) return;
        
        // Expand scrollable areas so nothing gets cut off
        ...
        // Take the picture (Now safely stripped of lab colors and CORS images)
        ...
        setIsExporting(false); // Brings the watermark and buttons back
      }
    }, 150); // 150ms gives React enough time to clear the DOM
  };
  ...
  const SkillRow = ({ name, base, type }: { name: string, base: number, type: string }) => {
    ...
    return (
      <tr className={`border-b border-gray-300 text-[10px] ${overCap ? 'bg-red-100' : ''}`}>
        <td className={`p-1 font-bold ${overCap ? 'text-red-600' : ''}`}>{name}</td>
        <td className="text-center text-gray-500">{base}</td>
        <td className="bg-blue-50 text-center border-l border-gray-200">
        ...
        <td className="bg-green-50 text-center border-l border-gray-200">
        ...
        <td className="bg-purple-50 text-center border-l border-gray-200">
        ...
        <td className="text-center font-bold border-l border-gray-200">{total}%</td>
      </tr>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div ref={sheetRef} className={`max-w-7xl mx-auto border-4 border-black p-6 relative overflow-hidden ${isExporting ? 'bg-white' : 'bg-white shadow-[0_0_50px_rgba(0,0,0,0.2)]'}`}>
        
        {/* React Trick: Only render watermark if NOT exporting */}
        {!isExporting && (
          <div 
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10">
          ...
          <section className="space-y-4">
            <div className="border-2 border-black p-2 bg-white">
              ...
              <input placeholder="Character Name" className="w-full text-xs border border-gray-300 p-1 mb-1 bg-white" value={name} onChange={e => setName(e.target.value)} />
              <div className="grid grid-cols-2 gap-1">
                <select className="text-xs border border-gray-300 p-1 bg-white" value={race} onChange={e => setRace(e.target.value)}>
                    <option>Human</option><option>Melnibonéan</option><option>Myyrrhn</option>
                </select>
                <div className="flex items-center gap-1 border border-gray-300 p-1 bg-white">
            ...
            <div className="border-2 border-black p-2 bg-amber-50">
              <h2 className="font-bold border-b border-black mb-1 uppercase text-[10px] flex justify-between font-black">
                Step 2: Background {!isExporting && <button onClick={() => setBackground(backgroundEvents[Math.floor(Math.random()*backgroundEvents.length)])} className="bg-black text-white px-1 text-[8px] hover:bg-red-900 transition-colors">Roll</button>}
              </h2>
            ...
            <div className="border-2 border-black p-2 bg-blue-50">
              ...
              <select className="w-full text-xs border border-gray-300 p-1 bg-white" value={selectedCulture} onChange={e => setSelectedCulture(e.target.value)}>
            ...
            <div className="border-2 border-black p-2 bg-green-50">
              ...
              <select className="w-full text-xs border border-gray-300 p-1 bg-white" value={selectedCareer} onChange={e => setSelectedCareer(e.target.value)}>
          ...
          <section className="space-y-4">
            <div className="border-2 border-black p-2 bg-white">
              ...
              <div className="grid grid-cols-2 gap-x-4">
                {(Object.keys(characteristics) as (keyof Characteristics)[]).map(k => (
                  <div key={k} className="flex justify-between items-center text-xs mb-1 border-b border-gray-300">
            ...
            <div className="border-2 border-black p-2 bg-stone-100">
              <h2 className="font-bold border-b border-black mb-2 uppercase text-[10px] font-black">Step 4: Attributes</h2>
              <div className="text-[10px] space-y-1">
                <div className="flex justify-between border-b border-gray-300"><span>Action Points:</span> <strong>{actionPoints}</strong></div>
                <div className="flex justify-between border-b border-gray-300"><span>Damage Modifier:</span> <strong>{damageMod}</strong></div>
                <div className="flex justify-between border-b border-gray-300"><span>Initiative Bonus:</span> <strong>{initiative}</strong></div>
                <div className="flex justify-between border-b border-gray-300"><span>Healing Rate:</span> <strong>{healingRate}</strong></div>
                <div className="flex justify-between border-b border-gray-300"><span>Luck Points:</span> <strong>{luck}</strong></div>
                <div className="flex justify-between border-b border-gray-300"><span>Movement:</span> <strong>{movement}</strong></div>
                <div className="flex justify-between border-b border-gray-300 text-blue-900 font-black"><span>Tenacity (POW):</span> <strong>{characteristics.POW}</strong></div>
            ...
            <div className="border-2 border-black p-2 bg-red-50">
              ...
              <table className="w-full text-center text-[9px] uppercase tracking-wider">
                <thead className="border-b border-gray-400 text-gray-600"><tr><th>D20</th><th className="text-left">Location</th><th>HP</th></tr></thead>
                <tbody>
                  <tr className="border-b border-gray-300"><td>1-3</td><td className="text-left font-bold">Right Leg</td><td className="font-black">{hpBase}</td></tr>
                  <tr className="border-b border-gray-300"><td>4-6</td><td className="text-left font-bold">Left Leg</td><td className="font-black">{hpBase}</td></tr>
                  <tr className="border-b border-gray-300"><td>7-9</td><td className="text-left font-bold">Abdomen</td><td className="font-black">{hpBase + 1}</td></tr>
                  <tr className="border-b border-gray-300"><td>10-12</td><td className="text-left font-bold">Chest</td><td className="font-black">{hpBase + 2}</td></tr>
                  <tr className="border-b border-gray-300"><td>13-15</td><td className="text-left font-bold">Right Arm</td><td className="font-black">{Math.max(1, hpBase - 1)}</td></tr>
                  <tr className="border-b border-gray-300"><td>16-18</td><td className="text-left font-bold">Left Arm</td><td className="font-black">{Math.max(1, hpBase - 1)}</td></tr>
            ...
            <div className="border-2 border-black p-2 bg-purple-50">
              ...
              {passions.map((p, idx) => (
                <div key={p.id} className="flex gap-1 mb-1 items-center">
                  <input className="text-[10px] border border-gray-300 flex-1 p-1 bg-white" placeholder="Love / Hate..." value={p.target} 
                    onChange={e => { const n = [...passions]; n[idx].target = e.target.value; setPassions(n); }} />
                  <span className="text-[10px] font-black p-1 bg-white border border-gray-300 min-w-[35px] text-center">{characteristics.POW + characteristics.CHA}%</span>
                  {!isExporting && <button onClick={() => setPassions(passions.filter(x => x.id !== p.id))} className="text-red-600 hover:text-black transition-colors font-bold text-xs">×</button>}
                </div>
              ))}
              {!isExporting && <button onClick={() => setPassions([...passions, {id: Date.now(), target: "", val: 0}])} className="text-[8px] bg-black text-white w-full py-1 mt-1 uppercase hover:bg-purple-900 transition-colors">+ Add Passion</button>}
            </div>
          </section>

          {/* COLUMN 3 & 4: SKILLS TABLE */}
          <section className="md:col-span-2 border-2 border-black flex flex-col bg-white">
            ...
            <div className="overflow-y-auto max-h-[850px] p-2">
              <table className="w-full border-collapse">
                <thead className="text-[9px] uppercase border-b border-black text-left">
                  <tr><th className="p-1">Skill</th><th>Base</th><th className="bg-blue-100 text-center">C</th><th className="bg-green-100 text-center">J</th><th className="bg-purple-100 text-center">B</th><th className="text-center font-black">Total</th></tr>
                </thead>
                <tbody>
                  {standardSkillKeys.map(k => <SkillRow key={k} name={k} base={getStandardBase(k)} type="standard" />)}
                  <tr className="bg-gray-200 text-[9px] font-bold text-center uppercase tracking-widest"><td colSpan={6} className="py-1">Professional Skills</td></tr>
